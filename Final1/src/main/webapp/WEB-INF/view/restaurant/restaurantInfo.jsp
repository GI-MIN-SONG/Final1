...
<table class="table table-striped">
	...
	<tr>
		...
		<c:if test="${empty loginmember}" var="result">
			<button type="button" onclick="writeReviewBtn_click();">간단 리뷰 작성</button> <!-- 로그인 하지 않으면 사용할 수 없음 alert 창 -->
		</c:if>
		<c:if test="${not result}">
			<button type="button" onclick="location.href='${ pageContext.request.contextPath }/allRestaurantList/${restaurant_id}/writeSimpleReview'">간단 리뷰 작성</button>
...
<hr>
   ...
   <div id="button">
      <c:if test="${ loginmember.member_id eq reviewList.member_id }">
         <button type="button" onclick="location.href='${ pageContext.request.contextPath }/allRestaurantList/${ reviewList.restaurant_id }/update/${reviewList.simple_review_id}'">수정</button>
         <button type="button" onclick="location.href='${ pageContext.request.contextPath }/allRestaurantList/${ reviewList.restaurant_id }/delete/${reviewList.simple_review_id}'">삭제</button>
      </c:if>   
   </div>
   
   <table class="table table-striped">
      <tr>
         <td width="200px" style="text-align: center">${ reviewList.member_nickname }</td>
         <td colspan="2">${ reviewList.simple_review_registdate }</td>
      </tr>
      <tr>
         <td><img src="${ pageContext.request.contextPath }/resources/img/${ reviewList.member_photo }" id="member" class="img-circle" width="100px" height="100px"></td>
